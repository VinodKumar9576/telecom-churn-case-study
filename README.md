...
Below are the steps followed to build the model:

1. Preprocess data (convert columns to appropriate formats, handle missing values, etc.)
2. Conduct appropriate exploratory analysis to extract useful insights (whether directly useful for business or for eventual modelling/feature engineering).
3. Feature Engineering : Derive new features.
4. Use PCA to reduce the number of predctors.
...
7. Finally, choose a model based on appropriate evaluation metrics.

The above model will only be able to achieve one of the two goals:
* To predict customers who will churn. We can’t use the above model to identify the important features for churn. That’s because PCA usually creates components which are not easy to interpret.
